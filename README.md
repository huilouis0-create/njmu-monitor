...
- 每个列表页最多尝试 3 次，单次请求最长等待 30 秒。
- 请求创建时即设置连接超时，覆盖 DNS/TCP/TLS 建连阶段，避免 Node 默认 Agent 的 5 秒超时提前中断请求；另保留整个请求的总时限。
- 超时日志记录实际耗时、配置时限、主机和失败阶段，便于区分连接、等待响应和正文读取问题。连续失败本身不能证明学校网站宕机。
- 单轮临时超时只写入 `state.json`，不推送故障消息，也不让 Action 失败。
- 同一站点连续 2 轮失败后才推送一次“持续异常”，后续运行仍会自动重试。
- 网站恢复访问后自动清零故障计数。
- 详情页内容指纹属于辅助检查，使用独立的短超时，不会长期拖住整轮任务。
...
| `monitor.test.js` | 慢连接/请求超时回归、解析器与连续故障判定测试 |
| `pushplus.js` | PushPlus 微信推送工具模块 |
| `state.json` | 自动生成，记录已通知 URL 与站点连续故障状态 |
